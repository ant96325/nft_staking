...
describe("NFTStaking.sol", () => {
    ...
    beforeEach(async () => {
        ...
        // Deploy AntNFT NFT contract 
        const NFTContract = await ethers.getContractFactory("AntNFT");
        nftContract = await NFTContract.deploy(maxSupply, mintCost, maxMintAmount);

        // Deploy AntNFT ERC20 token contract 
        const TokenContract = await ethers.getContractFactory("AntToken");
    ...
    describe("Correct Deployement", () => {
        ...
        it("NFT contract should have correct initial parameters", async () => {
            ...
            await expect(nftContract.tokenURI(1)).to.be.revertedWithCustomError(nftContract, 'AntNFT__QueryForNonExistentToken');
        });
    ...
    describe("Core Functions", () => {
        ...
        it("should not allow user to mint NFT while contract is paused", async () => {
            ...
            await expect(
                ...
                nftContract, "AntNFT__ContractIsPaused"
            )
        });
        ...
        it("should not allow random users to mint KPT tokens", async () => {
            ...
            ).to.be.revertedWithCustomError(tokenContract, "AntToken__OnlyControllersCanMint")
        });
    });
